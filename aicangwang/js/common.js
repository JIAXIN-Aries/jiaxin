...
(function($) {
    $.show_nc_login = function(options) {
        var settings = $.extend({}, {action:'/index.php?act=login', nchash:'', formhash:'' ,anchor:''}, options); // index.php?act=member&op=login
        var login_dialog_html = ""; 
        login_dialog_html += '<div class="quick-login">';
        login_dialog_html += '<form class="bg" method="post"  action="'+SITEURL+settings.action+'">';
        login_dialog_html += '<input type="hidden" value="ok" name="form_submit">';
        login_dialog_html += '<input type="hidden" value="'+settings.formhash+'" name="formhash">';
        login_dialog_html += '<input type="hidden" value="'+settings.nchash+'" name="nchash">';
        login_dialog_html += '<dl style=" margin-top:-10px;"><dt>用户名</dt><dd><input type="text" name="user_name" autocomplete="off" class="text"></dd></dl>';
        login_dialog_html += '<dl><dt>密&nbsp;&nbsp;&nbsp;码</dt><dd><input type="password" autocomplete="off" name="password" class="text"></dd></dl>';
        login_dialog_html += '<dl><dt>验证码</dt><dd><input type="text" size="10" maxlength="4" class="text fl w60" name="captcha"><img border="0" onclick="this.src=\''+APP_SITEURL+'/index.php?act=seccode&amp;op=makecode&amp;nchash='+settings.nchash+'&amp;t=\' + Math.random()" name="codeimage" title="看不清，换一张" src="'+APP_SITEURL+'/index.php?act=seccode&amp;op=makecode&amp;nchash='+settings.nchash+'" class="fl ml10"><span>不区分大小写</span></dd></dl>';
        login_dialog_html += '<ul><li>›&nbsp;如果您没有登录帐号，请先<a class="register" href="'+SITEURL+'/index.php?act=login&amp;op=register">注册会员</a>然后登录</li><li>›&nbsp;如果您<a class="forget" href="'+SITEURL+'/index.php?act=login&amp;op=forget_password">忘记密码</a>？，申请找回密码</li></ul>';
        var ref_url = document.location.href; 
        if(settings.anchor !== '') ref_url += '#'+settings.anchor;
        login_dialog_html += '<div class="enter"><input type="submit" name="Submit" value="&nbsp;" class="submit"></div><input type="hidden" name="ref_url" value="'+ref_url+'"></form></div>';
